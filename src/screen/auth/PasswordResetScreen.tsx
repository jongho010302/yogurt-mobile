...
const PasswordResetScreen: React.FC = () => {
  ...
  const confirmChangePasswordDialog = () => {
    Alert.alert('', '비밀번호를 변경하시겠습니까?', [
      {
        text: '취소',
        onPress: () => console.log('function confirmChangePasswordDialog: Click Cancel'),
      },
      {
        text: '확인',
  ...
  const isButtonEnabled = () => {
    return isPasswordValidated && isSecondPasswordValidated && password === secondPassword;
  };
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.white }}>
      <ScrollView style={styles.scrollView}>
        ...
        <View>
          ...
          />
          <BaseBottomText text={changePasswordMessage} color={colors.lightGray} />
        </View>
      </ScrollView>
      <BaseButton
